    ...
    stages {
        stage('Clean Workspace') {
            steps {
                ...
                // Check for changes in frontend, backend, and bd-api repositories
        stage('Check for Changes') {
            steps {
                script {
                    // Git fetch to get latest changes
                    sh 'git fetch'
                    ...
                    // Check if there are changes in the respective repositories
                    if (changes.contains('kepler-frontend/')) {
                        env.FRONTEND_CHANGED = 'true'
                    }
                    if (changes.contains('kepler-backend/')) {
                        env.BACKEND_CHANGED = 'true'
                    }
                    if (changes.contains('kepler-bd-api/')) {
